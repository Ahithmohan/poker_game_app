...
//     print(widget.playerResponse?.data?.holemGameVersion);
//   }
//
//   @override
//   void dispose() {
...
//         debugPrint(
//             'ZIP file is already downloaded and extracted. Skipping extraction.');
...
class _GameViewState extends State<GameView> {
  ...
  Future<void> _downloadAndExtractFile() async {
    ...
    try {
      ...
      // Parse version from player response
      final currentVersionStr = widget.playerResponse?.data?.holemGameVersion;
      final currentVersion = int.tryParse(currentVersionStr ?? '') ?? 0;

      final storedVersion = prefs.getInt('game_version');

      final isFirstTime = storedVersion == null;
      final isVersionMatch = storedVersion == currentVersion;

      if (isFirstTime) {
        debugPrint('No stored game version found. First-time setup.');
      } else if (!isVersionMatch) {
        debugPrint(
            'New version available: updating from v$storedVersion to v$currentVersion');
      } else {
        debugPrint('Game is up to date. Version: v$currentVersion');
      }
      ...
      final isExtracted = await File('$extractionPath/index.html').exists();
      final zipFile = File(zipFilePath);
      final isZipDownloaded = await zipFile.exists();

      if (!isFirstTime && isVersionMatch && isExtracted && isZipDownloaded) {
        debugPrint('Game version is up to date. Launching game directly.');
        await _startServer(extractionPath);
        setState(() => isLoading = false);
        return;
      }

      debugPrint('New game version or first-time launch. Updating files...');
      ...
      if (response.statusCode == 200) {
        await zipFile.writeAsBytes(response.bodyBytes);
        debugPrint('ZIP file downloaded.');
      } else {
      ...
      await _extractZip(zipFilePath, extractionPath);
      debugPrint('Extraction completed.');

      // Store new version
      await prefs.setInt('game_version', currentVersion);
      debugPrint('Stored game version: $currentVersion');

      await _startServer(extractionPath);
    } catch (e) {
      debugPrint('Error during download & extract: $e');
      if (mounted) {
